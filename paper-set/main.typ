...
#show: project.with(
  ...
  outl: [
    ...
    #outline(title: text(yellow.negate(space: rgb))[Revise], target: figure.where(kind: "thmenv")) <the-outline>
  ],
  title_page: true,
)
...
`18, 23`
#qst("18")[
  计算 $integral.double_(D) dif sigma$，其中 $D = {(x, y) | x^2 + y^2 <= 2, y >= x^2}$.
  #ps[对称性是根据被积函数使用的，不能只看积分区域是否对称。]
  #image("../resources/R-302-2015-18.png")
]
...
#qst("23")[
  ...
  #ps[概念不牢，为什么第二问等于求 $bold(A)$ 的线性无关的特征向量。]
]
...
#qst("3")[
  ...
  #ps[定积分实际上积分上下限就是积分变量的取值范围，对于反常积分，特别是 $e^x$ 相关的，极限在某点正负取值不同的反常积分，更应该注意积分区域的取值。]
]
...
#qst("23")[
  #ps[等于是压根不知道为什么要求，也不知道为什么要单位化，必须巩固二次型和特征值章节的知识了！]
]
