...
#remark("构造函数")[
  ...
  $"见到"[f(x)f'(x)],space space F(x) = f^2(x)$

  $"见到"[f'(x)]^2 + f(x)f''(x),space space F(x) = f(x)f'(x)$

  $"见到"f'(x) + f(x)phi'(x),space space F(x) = f(x)e^(phi(x))$

  $"见到" u''v + 2u'v' + 2u v'',space space (u v)''$

  $"见到" f'(x)x - f(x),space space F(x) = f(x) / x$

  $"见到"f''(x)f(x) - [f'(x)]^2,space space F(x) = (f'(x)) / f(x) "或" F(x) = ln f(x)$
