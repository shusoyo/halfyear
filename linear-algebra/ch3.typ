...
#annotation("线性表示与矩阵乘法的关系")[
  ...
  #ps[对于 $bold(A)$ 来说，右乘 $bold(B)$ 相当于对 $bold(A)$ 做了线性列变换，$bold(A B = C)$ 则 $bold(C)$ 的列是由 $bold(A)$ 经过一系列或可逆或不可逆的线性列变换得出的，所以 $bold(C)$ 的列可以由 $bold(A)$ 的列表出，同理 $bold(C)$ 的行可以被 $bold(B)$ 的行表出]
...
#proposition([矩阵秩的性质])[
  ...
  + 如果 $bold(A B) = bold(0)$, 则 $r (bold(A)) + r (bold(B)) lt.eq n$,
    ...
    更一般的： 对任意的 $bold(A\, B), r(bold(A B)) >= r(bold(A)) + r(bold(B)) - n$ \
